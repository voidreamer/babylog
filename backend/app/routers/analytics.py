...
async def get_baby_analytics(
    ...
    ).order_by(Feeding.time).all()
    
    # Today's feedings
    todays_feedings = [f for f in feedings if f.time >= today_start]
    ...
    todays_sleeps = [s for s in sleeps if s.start_time >= today_start and s.end_time]
    
    # Diapers in analysis window
    diapers = db.query(Diaper).filter(
        Diaper.baby_id == baby_id,
        Diaper.time >= start_date,
    ).order_by(Diaper.time).all()
    
    # Today's diapers
    todays_diapers = [d for d in diapers if d.time >= today_start]
    ...
    bedtimes = [
        s.start_time for s in sleeps 
        if s.start_time.hour >= 18 or s.start_time.hour <= 3
    ]
    usual_bedtime = calculate_average_time_of_day(bedtimes)
    
    # Nap durations (daytime sleep)
    nap_durations = [
        s.duration_minutes for s in sleeps 
        if s.duration_minutes 
        and 6 <= s.start_time.hour < 18
        and s.duration_minutes < 180  # Under 3 hours = nap
    ]
    ...
    birth_date = baby.birth_date.date() if baby.birth_date else None
