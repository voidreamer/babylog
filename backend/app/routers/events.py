...
from ..models import Bath, Diaper, Feeding, Potty, Pumping, Sleep, Solid, Supplement, TummyTime
...
def get_timeline(
    ...
    feedings = (
        db.query(Feeding)
        .filter(Feeding.baby_id == baby_id, Feeding.time >= start_of_day_utc, Feeding.time < end_of_day_utc)
        .all()
    )
    ...
    diapers = (
        db.query(Diaper)
        .filter(Diaper.baby_id == baby_id, Diaper.time >= start_of_day_utc, Diaper.time < end_of_day_utc)
        .all()
    )
    ...
    pumpings = (
        db.query(Pumping)
        .filter(Pumping.baby_id == baby_id, Pumping.time >= start_of_day_utc, Pumping.time < end_of_day_utc)
        .all()
    )
    ...
    for p in pumpings:
        events.append(
            TimelineEvent(
                ...
                event_type="pumping",
                time=p.time,
                details={"duration_minutes": p.duration_minutes, "amount_ml": p.amount_ml, "notes": p.notes},
            )
        )
    ...
    potty_logs = (
        db.query(Potty)
        .filter(Potty.baby_id == baby_id, Potty.time >= start_of_day_utc, Potty.time < end_of_day_utc)
        .all()
    )
    ...
    for pt in potty_logs:
        events.append(
            TimelineEvent(
                ...
                event_type="potty",
                time=pt.time,
                details={"result": pt.result, "potty_type": pt.potty_type, "notes": pt.notes},
            )
        )
    ...
    for tt in tummy_times:
        events.append(
            TimelineEvent(
                ...
                event_type="tummy",
                time=tt.start_time,
                details={"duration_minutes": tt.duration_minutes, "notes": tt.notes},
            )
        )

    # Baths
    baths = (
        db.query(Bath).filter(Bath.baby_id == baby_id, Bath.time >= start_of_day_utc, Bath.time < end_of_day_utc).all()
    )

    for b in baths:
        events.append(TimelineEvent(id=b.id, event_type="bath", time=b.time, details={"notes": b.notes}))

    # Supplements
    supplements = (
        db.query(Supplement)
        .filter(Supplement.baby_id == baby_id, Supplement.time >= start_of_day_utc, Supplement.time < end_of_day_utc)
        .all()
    )
    ...
    for sup in supplements:
        events.append(
            TimelineEvent(
                ...
                time=sup.time,
                details={"name": sup.name, "dosage": sup.dosage, "notes": sup.notes},
            )
        )
    ...
    solids = (
        db.query(Solid)
        .filter(Solid.baby_id == baby_id, Solid.time >= start_of_day_utc, Solid.time < end_of_day_utc)
        .all()
    )
...
def get_daily_summary_for_baby(db: Session, baby_id: int, date: datetime, tz_offset: int = 0) -> DailySummary:
    ...
    feedings = (
        db.query(Feeding)
        .filter(Feeding.baby_id == baby_id, Feeding.time >= start_of_day, Feeding.time < end_of_day)
        .all()
    )
    ...
    # Diapers
    diapers = (
        db.query(Diaper).filter(Diaper.baby_id == baby_id, Diaper.time >= start_of_day, Diaper.time < end_of_day).all()
    )
    ...
    for s in sleeps:
        if s.end_time is None:
            ...
            effective_end = min(datetime.now(UTC).replace(tzinfo=None), end_of_day)
        else:
    ...
    pumpings = (
        db.query(Pumping)
        .filter(Pumping.baby_id == baby_id, Pumping.time >= start_of_day, Pumping.time < end_of_day)
        .all()
    )
    ...
    # Potty
    potty_logs = (
        db.query(Potty).filter(Potty.baby_id == baby_id, Potty.time >= start_of_day, Potty.time < end_of_day).all()
    )
    ...
    tummy_times = (
        db.query(TummyTime)
        .filter(TummyTime.baby_id == baby_id, TummyTime.start_time >= start_of_day, TummyTime.start_time < end_of_day)
        .all()
    )
    ...
    # Baths
    baths = db.query(Bath).filter(Bath.baby_id == baby_id, Bath.time >= start_of_day, Bath.time < end_of_day).all()

    # Supplements
    supplements = (
        db.query(Supplement)
        .filter(Supplement.baby_id == baby_id, Supplement.time >= start_of_day, Supplement.time < end_of_day)
        .all()
    )

    # Solids
    solid_meals = (
        db.query(Solid).filter(Solid.baby_id == baby_id, Solid.time >= start_of_day, Solid.time < end_of_day).all()
    )
...
def get_dashboard(
    ...
    last_feeding = db.query(Feeding).filter(Feeding.baby_id == baby_id).order_by(Feeding.time.desc()).limit(1).first()

    last_diaper = db.query(Diaper).filter(Diaper.baby_id == baby_id).order_by(Diaper.time.desc()).limit(1).first()

    last_sleep = (
        db.query(Sleep)
        .filter(Sleep.baby_id == baby_id, Sleep.end_time.isnot(None))
    ...
    last_pumping = db.query(Pumping).filter(Pumping.baby_id == baby_id).order_by(Pumping.time.desc()).limit(1).first()

    last_potty = db.query(Potty).filter(Potty.baby_id == baby_id).order_by(Potty.time.desc()).limit(1).first()

    last_tummy = (
        db.query(TummyTime).filter(TummyTime.baby_id == baby_id).order_by(TummyTime.start_time.desc()).limit(1).first()
    )

    last_bath = db.query(Bath).filter(Bath.baby_id == baby_id).order_by(Bath.time.desc()).limit(1).first()

    last_supplement = (
        db.query(Supplement).filter(Supplement.baby_id == baby_id).order_by(Supplement.time.desc()).limit(1).first()
    )

    last_solid = db.query(Solid).filter(Solid.baby_id == baby_id).order_by(Solid.time.desc()).limit(1).first()
    ...
    if local_date:
        try:
            ...
            summary_date = datetime.now(UTC).replace(tzinfo=None)
    else:
        summary_date = datetime.now(UTC).replace(tzinfo=None)
