...
// MARK: - Bath Request Body

private struct BathRequest: Encodable {
    let babyId: Int
    let time: String
    let notes: String?
...
struct BathFormView: View {
    ...
    init(babyId: Int, existing: Bath? = nil, onSaved: (() -> Void)? = nil) {
        self.babyId = babyId
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle(isEditing ? "Edit Bath" : "Log Bath")
    ...
    private func save() async {
        ...
        do {
            if let existing, let id = existing.id.intValue {
                _ = try await api.updateBath(id: id, body)
            } else {
